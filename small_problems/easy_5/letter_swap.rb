=begin

Given a string of words separated by spaces, write a method that takes this string of words and returns a string 
in which the first and last letters of every word is swapped.

You may assume that every word contains at least one letter, and that the string will always contain at least one 
word. You may also assume that each string contains nothing but words and spaces

Examples:

swap('Oh what a wonderful day it is') == 'hO thaw a londerfuw yad ti si'
swap('Abcde') == 'ebcdA'
swap('a') == 'a'

=end



def swap(string)
  sentence_array = string.split
  my_new_string = ''

  sentence_array.each do |word|
    current_arry = word.chars     
    word_array = word.chars 
    word_array[0] = current_arry.last
    word_array[-1] = current_arry.first

    new_word = word_array.join
      if word == sentence_array.last
        my_new_string << new_word
      else
        my_new_string << new_word + ' '
      end
    end
    my_new_string
end


swap('Oh what a wonderful day it is') == 'hO thaw a londerfuw yad ti si'
swap('Abcde') == 'ebcdA'
swap('a') == 'a'
